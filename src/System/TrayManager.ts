...
export class TrayManager {
    ...
    private static openSettings(menuItem: MenuItem, BrowserWindow: BrowserWindow, event: Event) {
        const manager = ComponentManager.getManager();
        manager.loadApplicationComponent();
    }

    private static editMode(menuItem: MenuItem, BrowserWindow: BrowserWindow, event: Event) {
        const manager = ComponentManager.getManager();
        const currSettings = manager.getSettings();
        manager.updateSettings(JSON.stringify({
            editMode: !currSettings.editMode
        }), true);
    }

    private static exit(menuItem: MenuItem, BrowserWindow: BrowserWindow, event: Event) {
        app.exit(0);
    }
}
